...
public class TC102_VerifyRestAPICanCreateAnPublishByPublisherIT {
	String apiID;
	String endpoint;
	...
	void createApplication() {
		Response response = given().relaxedHTTPSValidation().auth().preemptive().basic("admin", "admin")
				...
				.body(getPayload())
				.when().post(endpoint + "/api/am/publisher/v3/apis?openAPIVersion=v3");
	...
	String getPayload() {
		return "{\"name\":\"PizzaShack\",\"version\":\"1.0.0\",\"context\":\"/pizzashack\",\"policies\":" +
				"[\"Unlimited\"],\"endpointConfig\":{\"endpoint_type\":\"http\",\"sandbox_endpoints\":" +
				"{\"url\":\"https://localhost:9443/am/sample/pizzashack/v1/api/\"},\"production_endpoints\":" +
